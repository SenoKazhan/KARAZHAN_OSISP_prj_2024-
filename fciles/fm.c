...
void getImgPreview(char *filepath, int maxy, int maxx)
{
    ...
    if (pid == 0)
    {
        execl(DISPLAYIMG,CLEARIMG,filepath,(char *)NULL);
        exit(1);
    }
...
/*
    Gets previews of PDF Documents. Evince 
*/
...
// Функция для открытия файла с помощью Evince
void openFileWithEvince(char *filepath) {
    char command[1024];
    snprintf(command, sizeof(command), "evince \"%s\"", filepath);
    system(command);
}
/*
...
void getPreview(char *filepath, int maxy, int maxx)
{
    ...
    if(strcasecmp("jpg",last) == 0 || strcasecmp("png",last) == 0 || strcasecmp("gif",last) == 0 || strcasecmp("jpeg",last) == 0 || strcasecmp("mp3",last) == 0)
    {
        ...
        clearFlagImg = 1;
        char command[1024];
        snprintf(command, sizeof(command), "feh \"%s\"", filepath);
        system(command);
    }
...
/*
   Rename files in clipboard. ПЕРЕДЕЛАТЬ с mv
*/
void renameFiles()
{
...
void displayDirectory(char* dir, WINDOW* win) {
    // Получение количества файлов в директории
    int len = getNumberofFiles(dir);

    // Выделение памяти для массива имен файлов
    char* directories[len];

    // Получение имен файлов в директории
    int status = getFiles(dir, directories);

    // Проверка на ошибку при получении файлов
    if (status == -1) {
        endwin();
    ...
    char* sort_dir;
    size_t allocSize = snprintf(NULL, 0, "%s", dir);
    ...
    maxy -= 2; // Оставляем две строки для status_win

    // Отображение содержимого директории в указанном окне
    int t = 0;
...
// Функция для освобождения памяти и завершения работы программы
void cleanup() {
  // Free Memory
    free(cache_path);
...
int main(int argc, char* argv[])
{
    ...
    do
    {
        ...
        init_windows();
         ...
         displayDirectory(dir, current_win);
